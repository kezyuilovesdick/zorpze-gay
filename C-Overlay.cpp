...
void d2oSetup(HWND tWindow) {
	...
	overlayWindow = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST,
		wc.lpszClassName, "Delusion", WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
...
void mainLoop() {
	...
	if (message.message != WM_QUIT)
	{
		...
		if (!IsIconic(overlayWindow)) {
			SetWindowPos(overlayWindow, HWND_TOPMOST, info.rcClient.left, info.rcClient.top, siz.width, siz.height, SWP_SHOWWINDOW);
			target->Resize(&siz);
		}
		target->BeginDraw();
...
void Setup(DirectOverlayCallback callback) {
